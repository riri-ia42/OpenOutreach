...
4. Renseigner dans `.env.production` (à la main, jamais via script) :
   ...
   # optionnel — défaut : dev_fusion~linkedin-profile-scraper
   EKOALU_APIFY_ACTOR=dev_fusion~linkedin-profile-scraper
   ```
   L'acteur par défaut est **à confirmer au test réel** : vérifier sur le
   store (https://apify.com/store, chercher "linkedin profile scraper" +
   "no cookies") qu'il est bien cookieless et son prix.
