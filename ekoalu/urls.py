"""URLs de l app ekoalu."""
from __future__ import annotations

from django.urls import path

from ekoalu import views
from ekoalu.monitoring import views as monitoring_views

app_name = "ekoalu"

urlpatterns = [
    path("", views.dashboard, name="dashboard"),
    path("messages/", views.outbound_list, name="outbound_list"),
    path("messages/<int:pk>/", views.outbound_detail, name="outbound_detail"),
    path("messages/<int:pk>/sortir-prospect/", views.outbound_sortir_prospect, name="outbound_sortir_prospect"),
    path("messages/<int:pk>/sortir-societe/", views.outbound_sortir_societe, name="outbound_sortir_societe"),
    path("sorties/", views.sorties_list, name="sorties_list"),
    path("rdv/", views.rdv_list, name="rdv_list"),
    path("rdv/<int:pk>/brief/", views.rdv_brief, name="rdv_brief"),
    path("rdv/<int:pk>/deck/", views.rdv_deck, name="rdv_deck"),
    path("rdv/<int:pk>/deck.pdf", views.rdv_deck_pdf, name="rdv_deck_pdf"),
    path("consignes/", views.consignes_list, name="consignes_list"),
    path("campaigns/", views.campaigns_list, name="campaigns_list"),
    path("campaigns/<int:pk>/", views.campaign_detail, name="campaign_detail"),
    path("leads/add/", views.leads_add, name="leads_add"),
    path("leads/<str:slug>/", views.lead_detail, name="lead_detail"),
    path("companies/", views.companies_list, name="companies_list"),
    path("inbox/", views.inbox, name="inbox"),
    path("inbox/reply/<int:pk>/action/", views.email_reply_action, name="email_reply_action"),
    path("companies-validation/", views.companies_validation, name="companies_validation"),
    path("deals/", views.deals_filtered, name="deals_filtered"),
    path("usage/", views.usage, name="usage"),
    path("budget/resume/", views.budget_resume, name="budget_resume"),
    path("emergency-stop/", views.emergency_stop_toggle, name="emergency_stop"),
    path("recap/", views.daily_recap_today, name="recap_today"),
    path("recap/<str:day>/", views.daily_recap_view, name="recap_day"),
    # Monitoring live
    path("health.json", monitoring_views.health_json, name="health_json"),
    path("live/", monitoring_views.live_dashboard, name="live_dashboard"),
]
