...
- **`browser/session.py`** — `AccountSession`: linkedin_profile, page, context, browser (None en mode persistant), playwright. `campaigns` cached_property. `ensure_browser()` launches/recovers browser. `self_profile` cached_property. **Refonte anti-détection 11/06** : `reauthenticate()` et `_maybe_refresh_cookies()` sont des **no-op** — on ne re-logge JAMAIS automatiquement (chaque re-login mot de passe sur un 401 déclenchait le checkpoint LinkedIn). Sur échec auth → STOP (`auth_watch`) + fenêtre laissée ouverte pour login manuel.
...
- **`browser/login.py`** — **Refonte 11/06 (anti-détection)** : `launch_persistent_browser()` lance le vrai **Chrome** (`channel="chrome"`) via **Patchright** (pas Playwright stock) en **`launch_persistent_context`** sur `data/linkedin_profile/` (`headless=False`, `no_viewport=True`, zéro override). `start_browser_session()` vérifie que le profil persistant est authentifié (`is_authenticated()` → /feed) ; sinon `_handle_login_failure()` = STOP + fenêtre ouverte, **aucune soumission auto de credentials**. Premier login = manuel via `manage.py linkedin_manual_login` (Richard coche « Rester connecté » → cookie `li_rm` device-trust 1 an). Voir `docs/MODE_OPERATOIRE_LINKEDIN.md`.
...
- **`conf.py:CAMPAIGN_CONFIG`** — `min_ready_to_connect_prob` (0.9), `min_positive_pool_prob` (0.20), `connect_delay_seconds` (10), `connect_no_candidate_delay_seconds` (300), `check_pending_recheck_after_hours` (24), `check_pending_jitter_factor` (0.2), `qualification_n_mc_samples` (100), `enrich_min_delay_seconds` (20), `enrich_max_delay_seconds` (45), `enrich_max_per_page` (10), `burst_min_seconds` (2700), `burst_max_seconds` (3900), `break_min_seconds` (600), `break_max_seconds` (1200), `min_action_interval` (120), `embedding_model` ("BAAI/bge-small-en-v1.5").
- **Prompt templates** (at `linkedin/templates/prompts/`) — `qualify_lead.j2` (temp 0.7), `search_keywords.j2` (temp 0.9), `follow_up_agent.j2` + `follow_up_agent_user.j2` (the follow-up prompt is split campaign-rubric / lead-data so the rubric is a cacheable prefix — see `docs/follow_up_agent.md`).
